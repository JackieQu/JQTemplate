...
- (void)test2 {
    ...
    /** 获取对象基本属性 **/
    // 获取类名
    JQLog(@"类名：%s",class_getName(cls));
    
    // 获取父类名
    JQLog(@"父类名：%s",class_getName(class_getSuperclass(cls)));
    ...
    for (int i = 0; i < methodOutCount; i ++) {
        ...
        JQLog(@"%@",NSStringFromSelector(sel));
    }
    free(methods);
    ...
    for (int i = 0; i < clsMethodOutCount; i ++) {
        ...
        JQLog(@"获取类方法：%@",NSStringFromSelector(sel));
    }
    free(clsMethods);
    // 获取指定类方法同理
    /** 获取方法 **/
...
void TestMetaClass(id self, SEL _cmd) {
    ...
    JQLog(@"This objcet is %p", self);
    
    JQLog(@"Class is %@, super class is %@", [self class], [self superclass]);
    
    Class currentClass = [self class];
    
    for (int i = 0; i < 4; i++) {
        
        JQLog(@"Following the isa pointer %d times gives %p", i, currentClass);
    ...
    JQLog(@"NSObject's class is %p", [NSObject class]);
    
    JQLog(@"NSObject's meta class is %p", object_getClass([NSObject class]));
}
